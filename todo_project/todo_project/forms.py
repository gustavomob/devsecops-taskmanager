...
class RegistrationForm(FlaskForm):
    username = StringField(
        label="Username", validators=[DataRequired(), Length(min=3, max=10)]
    )
...
class UpdateUserPassword(FlaskForm):
    old_password = PasswordField(
        label="Enter Old Password", validators=[DataRequired()]
    )
    new_password = PasswordField(
        label="Enter New Password", validators=[DataRequired()]
    )
...
class UpdateTaskForm(FlaskForm):
    task_name = StringField(
        label="Update Task Description", validators=[DataRequired()]
    )
